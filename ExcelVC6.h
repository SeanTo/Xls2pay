...
// Version 1.2 (30 April 2006)
	// - Added operator<< to pass CExcelCell to an output stream.
	// - Added Print() to CExcelSheet to print the worksheet to an output stream.
	// - Change CExcelCell Get functions to const functions.
	// - Rename CExcelSheet functions RenameWorkSheet() to Rename().
...
// Version 1.7 (22 May 2006)
	...
	// - Fixed bug with CExcelSheet::Cell.
	// - Fixed bug with CExcel::UpdateWorksheets().
// Version 1.8 (23 May 2006)
	...
	// - Minor changes to CExcelCell:Set functions.
// Version 1.9 (24 May 2006)
	// - Changed name_ in Style from SmallString to LargeString.
	// - Fixed bug in CExcelCell::GetString and CExcelCell::GetWString.
	// - Minor changes to functions in CExcel and CExcelSheet which checks for unicode.
...
// Version 1.13 (1 August 2006)
	// - Changed CExcelCell::Get() so that it will get a stored double as an integer or vice versa if necessary.
	// - Changed CExcelCell::Get() so that it will not cause any errors if a string is empty.
	// - Changed CExcelCell::SetString() and CExcelCell::SetWString() so that it will not save an empty string.
...
// 2008年10月24日增加(By 醉书生)
#ifdef _ENABLE_MFC_
	int GetSheetList(CStringArray &arr);
	int GetSheetList(CComboBox &cmb);
#endif // _ENABLE_MFC_
////////////////////////////////////////////////////////////////////////////////
...
public: // Worksheet functions.
	...
	CExcelSheet* GetWorksheet(size_t sheetIndex);	///< Get a pointer to an Excel worksheet at the given index. Index starts from 0. Returns 0 if index is invalid.
	CExcelSheet* GetWorksheet(const char* name);	///< Get a pointer to an Excel worksheet that has given ANSI name. Returns 0 if there is no Excel worksheet with the given name.
	CExcelSheet* GetWorksheet(const wchar_t* name);	///< Get a pointer to an Excel worksheet that has given Unicode name. Returns 0 if there is no Excel worksheet with the given name.
	
	CExcelSheet* AddWorksheet(int sheetIndex=-1);	///< Add a new Excel worksheet to the given index. Name given to worksheet is SheetX, where X is a number which starts from 1. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
	CExcelSheet* AddWorksheet(const char* name, int sheetIndex=-1);	///< Add a new Excel worksheet with given ANSI name to the given index. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
	CExcelSheet* AddWorksheet(const wchar_t* name, int sheetIndex=-1);	///< Add a new Excel worksheet with given Unicode name to the given index. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
...
public:
	...
	vector<CExcelSheet> yesheets_;	///< Parsed Worksheets.
};
...
public:
	CExcelSheet(CExcel* excel, size_t sheetIndex);
...
public: // Cell functions
	...
	CExcelCell* Cell(size_t row, size_t col); ///< Return a pointer to an Excel cell. row and col starts from 0. Returns 0 if row exceeds 65535 or col exceeds 255.
...
public:
	...
	CString GetStr(BOOL bTrim = TRUE) const;
