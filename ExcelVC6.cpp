...
// 2008年10月24日增加(By 醉书生)
#ifdef _ENABLE_MFC_
int CExcel::GetSheetList(CStringArray &arr)
{
	// 枚举xls表单
	CString str;
	CExcelSheet *pSheet = NULL;
	
	for(size_t i = 0; i < GetTotalWorkSheets(); ++i)
	{
		pSheet = GetWorksheet(i);
		if(NULL == pSheet)
			break;
		
		str = pSheet->GetAnsiSheetName();
		if( str.IsEmpty() )
			str = pSheet->GetUnicodeSheetName();
...
int CExcel::GetSheetList(CComboBox &cmb)
{
	CStringArray arr;
	int cnt = GetSheetList(arr);
	for(int i = 0; i < cnt; ++i)
		cmb.AddString(arr[i]);
...
size_t CExcel::GetTotalWorkSheets()
{
...
CExcelSheet* CExcel::GetWorksheet(size_t sheetIndex)
{
...
CExcelSheet* CExcel::GetWorksheet(const char* name)
{
...
// Returns 0 if there is no Excel worksheet with the given name.
CExcelSheet* CExcel::GetWorksheet(const wchar_t* name)
{
	size_t maxWorksheets = yesheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
CExcelSheet* CExcel::AddWorksheet(int sheetIndex)
{
	size_t sheetNo = yesheets_.size() + 1;
	CExcelSheet* yesheet = 0;
	do
	{
		char sname[50];
...
// Worksheet is added to the last position if sheetIndex == -1. 
// Returns a pointer to the worksheet if successful, 0 if otherwise.
CExcelSheet* CExcel::AddWorksheet(const char* name, int sheetIndex)
{
	size_t maxWorksheets = yesheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
	...
	if (sheetIndex == -1)
	{
		...
		yesheets_.push_back(CExcelSheet(this, worksheets_.size()-1));
	...
	else
	{
		...
		yesheet = &*(yesheets_.insert(yesheets_.begin()+sheetIndex, CExcelSheet(this, sheetIndex)));
...
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
CExcelSheet* CExcel::AddWorksheet(const wchar_t* name, int sheetIndex)
{
	size_t maxWorksheets = yesheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
bool CExcel::DeleteWorksheet(size_t sheetIndex)
{
...
char* CExcel::GetAnsiSheetName(size_t sheetIndex)
{
...
wchar_t* CExcel::GetUnicodeSheetName(size_t sheetIndex)
{
	if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
	{
...
bool CExcel::GetSheetName(size_t sheetIndex, char* name)
{
	if (!(workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1))
	{
...
bool CExcel::GetSheetName(size_t sheetIndex, wchar_t* name)
{
	if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
	{
...
// Index starts from 0.
// Returns true if successful, false if otherwise.
bool CExcel::RenameWorksheet(size_t sheetIndex, const char* to)
{
	size_t maxWorksheets = yesheets_.size();
...
// Index starts from 0.
// Returns true if successful, false if otherwise.
bool CExcel::RenameWorksheet(size_t sheetIndex, const wchar_t* to)
{
	size_t maxWorksheets = yesheets_.size();
	if (sheetIndex < maxWorksheets)
	{
...
void CExcel::UpdateYExcelWorksheet()
{
	...
	for (size_t i=0; i<maxWorksheets; ++i)
	{
		yesheets_.push_back(CExcelSheet(this,i));
	}
}
...
void CExcel::UpdateWorksheets()
{
	...
	for (size_t s=0; s<maxWorksheets; ++s)
	{
		...
		for (size_t r=0, curRowBlock=0; r<maxRows; ++r)
		{
			...
			{
				CExcelCell* cell = yesheets_[s].Cell(r,c);
				...
				{		
					...
					switch(cellType)
					{
						case CExcelCell::INT:
						{
							...
							for (; cl<maxCols; ++cl)
							{
								CExcelCell* cellNext = yesheets_[s].Cell(r,cl);
...
/******************************************************************************/
CExcelSheet::CExcelSheet(CExcel* excel, size_t sheetIndex) : 
...
// Returns 0 if name is in Unicode format.
char* CExcelSheet::GetAnsiSheetName()
{
...
// Get the current worksheet name.
// Returns 0 if name is in Ansi format.
wchar_t* CExcelSheet::GetUnicodeSheetName()
{
	if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
	{
...
// Get the current worksheet name.
// Returns false if name is in Unicode format.
bool CExcelSheet::GetSheetName(char* name)
{
	if (!(excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1))
	{
...
// Get the current worksheet name.
// Returns false if name is in Ansi format.
bool CExcelSheet::GetSheetName(wchar_t* name)
{
	if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
	{
...
// Returns true if successful, false if otherwise.
bool CExcelSheet::Rename(const char* to)
{
...
// Returns true if successful, false if otherwise.
bool CExcelSheet::Rename(const wchar_t* to)
{
	size_t maxWorksheets = excel_->workbook_.boundSheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
void CExcelSheet::Print(ostream& os, char delimiter, char textQualifier)
{
	for (size_t r=0; r<maxRows_; ++r)
	{
		for (size_t c=0; c<maxCols_; ++c)
		{
			CExcelCell* cell = Cell(r,c);
			switch (cell->Type())
			{
...
size_t CExcelSheet::GetTotalRows()
{
	return maxRows_;
}

// Total number of columns in current Excel worksheet.
size_t CExcelSheet::GetTotalCols()
{
	return maxCols_;
}
...
CExcelCell* CExcelSheet::Cell(size_t row, size_t col)	
{
...
bool CExcelSheet::EraseCell(size_t row, size_t col)
{
...
/******************************************************************************/
CExcelCell::CExcelCell() : type_(UNDEFINED) {};
...
CString CExcelCell::GetStr(BOOL bTrim /*= TRUE*/) const
{
	CString str;
	if( WSTRING == Type() )
		str = CString(GetWString());
	else
